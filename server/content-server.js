...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === "POST" && pathname === "/api/scenes/delete") {
    const body = await collectJson(req);
    ...
    await Promise.all([writeJson(draftsPath, nextDrafts), writeJson(publishedPath, nextPublished)]);
    sendJson(res, 200, { sceneId, deletedDraft, deletedPublished });
    return;
  }
